...
const DeleteModal: React.FC<Props & DispatchProps & StateProps> = ({
  ...
  <>
    <CustomModal
      title={intl.messages['utils.delete']}
      open={open}
      onClose={() => { onClose(); }}
      centerTitle
    >
      <>
        <Box>
          <Box display="flex" flexDirection="column" alignItems="center" my="40px">
            <p>{intl.messages['routines.delete.description']}</p>
            <br />
            <p><b>{name}</b></p>
            <br />
            <p>{intl.messages['routines.delete.warning']}</p>
            <p>{intl.messages['routines.delete.subdescription']}</p>
          </Box>
        </Box>
